...
class Block(nn.Module):
    def __init__(self,inchannels,outchannels,dropout,kernel,bias,depth,mode,factor=2):
        ...
        if mode == 'down':
            ...
            self.conv1 = nn.ConvTranspose3d(inchannels,outchannels,4,stride=self.stride,padding=self.padding,bias=bias)
            self.conv2 = nn.ConvTranspose3d(inchannels,outchannels,4,stride=self.stride,padding=self.padding,bias=bias)
        elif mode == 'same':
